...
main =
  do e <- init
     ...
     win <- createWindow 2000 1200 "NanoVG" Nothing Nothing
     case win of
...
renderDemo c demoData mx my w h t =
  do drawEyes c (fromIntegral w - 250) 50 150 100 (realToFrac mx) (realToFrac my) (realToFrac t)
     ...
     myDraw c ((fromIntegral w)/2) 50 600
...
myDraw :: Context -> CFloat -> CFloat -> CFloat -> IO ()
myDraw c x y width =
  do save c

     beginPath c
     rect c x y width 100
     fillColor c (rgba 255 155 155 32)
     fill c

     beginPath c
     roundedRect c x (y + 120) width 100 20
     fillColor c (rgba 155 255 155 32)
     fill c

     bg <- linearGradient c x (y + 240) x (y+340) (rgba 50 50 50 200) (rgba 0 0 0 255) 
     bg1 <- linearGradient c x (y + 340) x (y+440) (rgba 0 0 0 255) (rgba 100 100 100 200)  
     beginPath c
     rect c x (y+240) width 100
     fillPaint c bg
     fill c

     beginPath c
     rect c x (y+340) width 100
     fillPaint c bg1
     fill c
